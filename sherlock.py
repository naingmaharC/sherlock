...
def txtFileReader(fileName):
    crimefile = open(fileName, 'r')
    result = []
    for line in crimefile.readlines():
        row = line.split(',')
        if row[len(row)-1].replace("\n", ""):
...
def main():
    ...
    parser.add_argument("--html",
                        ...
                        help="Output HTML File.For reading as web document"
                        )
    #add html output show only 200
    parser.add_argument("--exist",
                        action="store_true",  dest="exist", default=False,
                        help="Using with --html ,HTML Output only show exisit link"
                        )
    #add txt output 
    parser.add_argument("--txt",
                        action="store_true",  dest="text", default=False,
                        help="Output Txt File.For reading as txt document"
                        )
    ...
    if args.input:
        args.username = txtFileReader(args.input)
    ...
    for username in args.username:
        ...
        for site in results:
            ...
            if results[site]['exists'] == "yes":
                existSite += 1
                if results[site]['http_status'] == 403:
                    accessDenied += 1
            else:
                notFoundSite += 1
        endTime = datetime.now()
        totalTime =( endTime - startTime ) 
        print(Style.BRIGHT+Fore.CYAN+f"\nTotal Site : {totalSite}")
        print(Style.BRIGHT+Fore.GREEN+f"Exist Site : {existSite}")
        print(Style.BRIGHT+Fore.YELLOW+f"Assess Denied Site : {accessDenied}")
        print(Style.BRIGHT+Fore.RED+f"Not Found Site : {notFoundSite}")
        print(Style.BRIGHT+Fore.WHITE+f"Total Estimate Time : {totalTime} seconds")
        print("\n")
        if args.csv == True:
        ...
        if args.html == True:
            htmlOutputUp = """
                ...
                <body>
                    ...
                    <table class="table">
                        ...
                        <tbody>
            """
            htmlOutputUp = htmlOutputUp.format(name=username.capitalize())
            htmlOutpuMiddle = ""
            for site in results:
                htmlTrTag ="""
                    <tr>
                        <th style="color:#3949ab">{url_main}</th>
                        <th>{url_user}</th>
                        <th>{exists}</th>
                        <th>{http_status}</th>
                ...
                """
                if results[site]['exists'] == "yes":
                    url_user = f"<a href={ results[site]['url_user']}>{ results[site]['url_user']}</a>"
                else:
                    url_user = "<span> Not Found! </span>"

                htmlTrTag = htmlTrTag.format(
                    url_main =site,
                    url_user = url_user,
                    exists = results[site]['exists'],
                    http_status = results[site]['http_status'],
                    response_time_ms = results[site]['response_time_ms'])
                
                if args.exist:
                    if  results[site]['exists'] == "yes":
                        htmlOutpuMiddle = htmlOutpuMiddle + htmlTrTag
                else:
                    htmlOutpuMiddle = htmlOutpuMiddle + htmlTrTag
            ...
            htmlOutput = htmlOutputUp + htmlOutpuMiddle +htmlOutputDown
            html_report = open(username + ".html", "w")
            html_report.write(htmlOutput)
            html_report.close()

            #Browser open
            webbrowser.open('file://' + os.path.realpath(username + ".html"))
